...
- **Weekend generation mode default is strict-then-relaxed.** `generate_schedule(..., weekend_variant_mode=STRICT_THEN_RELAXED)` first attempts strict alternation across the whole horizon; if no variants survive, the relaxed retry only runs after `confirm_rotation_callback` approves it (the default callback declines). `STRICT_ONLY` never introduces rotation repeats — an infeasible weekend fails generation rather than silently relaxing. `RELAXED_ALLOWED` skips strict generation and uses relaxed generation immediately.
- **Rotation violations are attributed per candidate schedule.** Each `ScheduleVariant` records the pattern repeats introduced on its own branch (`variant.rotation_violations`); the scheduler-level `get_rotation_violation_history()` is rebuilt from the surviving variants after generation, deduplicated, so pruned branches and shared ancestry no longer inflate the counts.
...
- **Weekend variant branching is beam-capped.** Variant counts grow roughly as (valid pairs)^(weekends), and each survivor runs the full evaluation pipeline. After each weekend, the pool is pruned to `max_weekend_variants` (default 500; 0 = unlimited), keeping the variants with the fewest rotation repeats, the most even weekend spread, and the largest minimum weekend gap. Pruning logs a warning, and a generation that dead-ends after pruning suggests raising the limit.
- **Pre-window history counts toward weekday spacing.** Days worked in the `min_days_between_assignments` window immediately before the schedule start (from weekend history and the per-day `schedule_history` table) are seeded into each snapshot, so `min_days_between_assignments` is enforced across the window boundary rather than only inside it.
