...
| 6 | #6 — Migrate screens/dialogs from `ui/legacy.py` into existing wrappers | 🟡 | `claude/implement-phase-6-E4N03` | All dialogs migrated: `CompactSettingsDialog`, `SettingsDialog`, `RotationViolationDialog`, and `VariantReviewDialog` now own their implementations in `ui/dialogs/*.py` (the previous-session subclass-wrapper pattern has been replaced by direct ownership). Each dialog imports only Qt, `ToolDialog`, and (where needed) a lazy `from ..legacy import …` for `WrappedCheck`/`themed_icon`/`show_info`/`export_variants_calendar_html` so module-load order remains safe. `ui/dialogs/__init__.py` is now eager (the circular-import workaround `__getattr__` was removed since the dialogs no longer import from `ui.legacy` at module level). `ui/legacy.py` re-imports each dialog at the bottom for backward compatibility; class identity is preserved across every import path. The previous session's batch (workers, widget primitives, `ToolDialog`) remains in place: `ScheduleProgressWorker`/`RebuildViolationWorker` in `ui/worker_threads.py`; `PinkHeaderView`/`MultiLineHeaderView`/`TallHeaderView` in `ui/widgets/header_views.py`; `MultiDatePickerGrid`/`MultiDatePicker`/`SingleDatePicker` in `ui/widgets/date_pickers.py`; `ToolDialog` in `ui/dialogs/tool_dialog.py`. `ui/legacy.py` shrank from 4,530 → 3,681 lines (-849 this session, -1,524 cumulative). The eight screens (`MainMenu`, `NurseManagementScreen`, `PreScheduledScreen`, `AssignmentHistoryScreen`, `ViewAllUnavailableScreen`, `WeekendHistoryCalendarScreen`, `ScheduleGenerationScreen`, `AdvancedWeekendStatsScreen`) remain to be migrated in subsequent sessions — they depend on `UiStyle`, `AppSettings`, and various module-level helpers that warrant a per-screen migration. |
...
- [x] Dialogs: `Settings`, `CompactSettings`, `ToolDialog`, `VariantReview`, `RotationViolation` — all dialogs now own their implementations directly in `ui/dialogs/*.py`. `ToolDialog` was migrated in the prior session; this session moved the remaining four subclass dialogs: `CompactSettingsDialog` (`ui/dialogs/compact_settings_dialog.py`), `SettingsDialog` (`ui/dialogs/settings_dialog.py`), `RotationViolationDialog` (new file `ui/dialogs/rotation_violation_dialog.py` + matching `_widget.py` re-export), `VariantReviewDialog` (`ui/dialogs/variant_review_dialog.py`). The previous subclass-wrapper hooks (`_build_layout`/`_bind_signals`/`_apply_initial_state`) have been removed since the wrapper file is the implementation. `ui/dialogs/__init__.py` exports all five dialogs eagerly (the circular-import `__getattr__` workaround is no longer needed). `ui/legacy.py` re-imports each dialog at the bottom for backward compatibility — class identity is preserved across `ui.legacy.X`, `ui.dialogs.X`, and `ui.dialogs.x_widget.X` (asserted by `tests/test_ui_import_smoke.py::test_ui_wrapper_modules_point_to_concrete_modules`, expanded to cover the new `RotationViolationDialog` widget and to assert `ui.legacy.X is ui.dialogs.x.X` for every dialog).
...
- [ ] `ui/legacy.py` reduced to compatibility shim (or deleted) — workers + widgets + `ToolDialog` brought it from 5,205 → 4,530 lines (-675); this session's dialog batch (`CompactSettings`, `Settings`, `RotationViolation`, `VariantReview`) drove it to 3,681 lines (-849 in this session; -1,524 cumulative). Remaining work tracked in the screen rows above; once those are migrated `ui/legacy.py` can be reduced to a thin re-export shim.
