...
| 6 | #6 — Migrate screens/dialogs from `ui/legacy.py` into existing wrappers | 🟡 | `claude/implement-phase-6-O4tlA` | First batch landed: `ScheduleProgressWorker` and `RebuildViolationWorker` now own their implementations in `ui/worker_threads.py`; `ui/legacy.py` re-imports them at the bottom for backward compat. `ui/legacy.py` shrank from 5,205 → 5,060 lines and the workers no longer carry their legacy-module address (`__module__ == "ui.worker_threads"`). Screens, dialogs, date pickers, and header views remain to be migrated in subsequent sessions. |
...
- [ ] `ui/legacy.py` reduced to compatibility shim (or deleted) — first batch of -145 lines (5,205 → 5,060); remaining work tracked in the screen/dialog rows above
