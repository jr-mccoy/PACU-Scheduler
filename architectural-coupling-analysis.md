...
| 6 | #6 — Migrate screens/dialogs from `ui/legacy.py` into existing wrappers | 🟡 | `claude/implement-phase-6-O4tlA` | Workers, widget primitives, and the base dialog migrated: `ScheduleProgressWorker`/`RebuildViolationWorker` now own their implementations in `ui/worker_threads.py`; `PinkHeaderView`/`MultiLineHeaderView`/`TallHeaderView` moved to `ui/widgets/header_views.py`; `MultiDatePickerGrid`/`MultiDatePicker`/`SingleDatePicker` moved to `ui/widgets/date_pickers.py`; `ToolDialog` (base class for every dialog) now lives in `ui/dialogs/tool_dialog.py` and is imported back at the top of `ui/legacy.py` so the subclass dialogs defined later can still extend it. `ui/dialogs/__init__.py` was made lazy (`__getattr__`) to break a circular import. Class identity is preserved across every import path (verified by `test_ui_wrapper_modules_point_to_concrete_modules`). `ui/legacy.py` shrank from 5,205 → 4,530 lines (-675). Subclass dialogs (`Settings`, `CompactSettings`, `RotationViolation`, `VariantReview`) and the eight screens remain to be migrated in subsequent sessions. |
...
- [ ] Dialogs: `Settings`, `CompactSettings`, `ToolDialog`, `VariantReview`, `RotationViolation` — `ToolDialog` migrated to `ui/dialogs/tool_dialog.py` (subclass-wrapper pattern removed; the file now owns the implementation directly, and `ui/legacy.py` imports it back early so `CompactSettingsDialog`/`SettingsDialog`/`RotationViolationDialog`/`VariantReviewDialog` defined later in `ui/legacy.py` still extend it). `ui/dialogs/__init__.py` switched to lazy `__getattr__` to break the resulting circular import. Remaining subclass dialogs (`Settings`, `CompactSettings`, `RotationViolation`, `VariantReview`) still live in `ui/legacy.py` and will be migrated next.
...
- [ ] `ui/legacy.py` reduced to compatibility shim (or deleted) — workers + widgets + `ToolDialog` batches brought it from 5,205 → 4,530 lines (-675); remaining work tracked in the screen/dialog rows above
