...
| 6 | #6 — Migrate screens/dialogs from `ui/legacy.py` into existing wrappers | 🟡 | `claude/implement-phase-6-O4tlA` | Workers + widget primitives migrated: `ScheduleProgressWorker`/`RebuildViolationWorker` now own their implementations in `ui/worker_threads.py`; `PinkHeaderView`/`MultiLineHeaderView`/`TallHeaderView` moved to `ui/widgets/header_views.py`; `MultiDatePickerGrid`/`MultiDatePicker`/`SingleDatePicker` moved to `ui/widgets/date_pickers.py`. `ui/legacy.py` re-imports all of them at the bottom for backward compat, and class identity is preserved (verified by `test_ui_wrapper_modules_point_to_concrete_modules`). `ui/legacy.py` shrank from 5,205 → 4,668 lines (-537). Screens and dialogs remain to be migrated in subsequent sessions. |
...
- [ ] `ui/legacy.py` reduced to compatibility shim (or deleted) — workers + widgets batch brought it from 5,205 → 4,668 lines (-537); remaining work tracked in the screen/dialog rows above
