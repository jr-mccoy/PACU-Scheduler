...
right after the first weekday assignment pass), ``gaps`` (what remained
after gap-fill and rebalancing) and ``unfillable`` (slots no nurse could
legally take, which no search can close). The report lines the two up so a manager
...
def write_gap_report(candidates: Iterable, path: str | os.PathLike[str]) -> str:
    ...
    """
    rows = []
    impossible: dict[int, list[str]] = {}
    for candidate in candidates:
        idx, stats = candidate[0], candidate[1]
        early = stats.get("early_gaps")
        final = stats.get("gaps")
        unfillable = stats.get("unfillable")
        filled = early - final if isinstance(early, int) and isinstance(final, int) else None
        rows.append((int(idx), early, final, filled, unfillable))
        if stats.get("unfillable_slots"):
            impossible[int(idx)] = list(stats["unfillable_slots"])
    rows.sort()
    ...
    lines = [
        ...
        "Unfillable = slots no nurse could legally take (included in Final).",
        "",
        f"{'Variant':>8}  {'Initial':>8}  {'Final':>8}  {'Filled':>8}  {'Unfillable':>10}",
    ]
    lines += [
        f"{idx + 1:>8}  {cell(early):>8}  {cell(final):>8}  {cell(filled):>8}  {cell(unf):>10}"
        for idx, early, final, filled, unf in rows
    ]
    if rows:
    ...
    if impossible:
        lines += ["", "Slots no nurse could take (time off, spacing or weekend rules):"]
        lines += [
            f"  Variant {idx + 1}: {', '.join(slots)}" for idx, slots in sorted(impossible.items())
        ]
